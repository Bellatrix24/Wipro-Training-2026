...
The CI/CD workflow is configured in `.github/workflows/ci-cd.yml`. It runs automatically when you push or make a pull request to the `main` branch. It sets up Node 20, installs dependencies, runs backend tests, and builds the frontend React project. Staging and production deployment steps run automatically only after all builds and tests pass on a push to `main`.
